...
class User extends Common
{
    ...
    public function login()
    {
        try {
            ...
            $data = $this->userService->login($account, $pwd, $plateformId, UserType::CUSTOMER);
    ...
    public function smsLogin()
    {
        try {
            ...
            if ($this->smsService->auditCode($tel, $type, $code)) {
                $data = $this->userService->smsLogin($tel, $plateformId, UserType::CUSTOMER);
                if (empty($data)) {
    ...
    protected function loginSuccess(UserModel $user, TokenModel $tokenModel)
    {
        try{
            //判断用户类型
            if ($user->getAttr('user_type') !== UserType::CUSTOMER) {
